...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        my_operator = self.operator.eval(env)
        my_operands = [a.eval(env) for a in self.operands]
        # try:
        return my_operator.apply(my_operands)
        # except:
        #     raise NameError('Exception raised!')
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        my_env = self.parent.copy()
        my_env.update(dict(zip(self.parameters, arguments)))
        return self.body.eval(my_env)
