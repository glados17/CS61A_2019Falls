...
def balanced(m):
    ...
    if is_weight(m) :
        ...
        is_balanced = total_weight(end(left(m)))*length(left(m)) == total_weight(end(right(m)))*length(right(m))
        return is_balanced and balanced(end(left(m))) and balanced(end(right(m)))
...
def make_fib():
    ...
    def make_fib_helper():
        ...
        if n <= 1:
            ...
            last_1, last_2, n = last_1 + last_2, last_1, n + 1
            return last_1
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,pwd):
        ...
        nonlocal errors
        if len(errors) >= 3:
            return 'Your account is locked. Attempts: ' + str(errors)
        elif pwd == password:
            if amount > balance:
...
def multiple_references_explanation():
    return """The multiple reference problem is so tough!"""
