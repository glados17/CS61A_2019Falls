...
def make_counter():
    ...
    "*** YOUR CODE HERE ***"
    counter_dict = {}
    def counter(str_to_count):
        if str_to_count in counter_dict:
            counter_dict[str_to_count] += 1
        else:
            counter_dict[str_to_count] = 1
        return counter_dict[str_to_count]
    return counter
...
class VendingMachine:
    ...
    def restock(self,n):
        self.storage += n
        return 'Current {0} stock: {1}'.format(self.product,self.storage)
...
def preorder(t):
    ...
    "*** YOUR CODE HERE ***"
    tree_list = []
    def get_tree_list(label):
        tree_list.append(label)
        return tree_list
    t.map(get_tree_list)
    return tree_list
...
def generate_paths(t, x):
    ...
    "*** YOUR CODE HERE ***"
    # learned form others, the for path in generate_paths(b, x) is awsome!!
    if t.label == x:
        yield [x]
...
def remove_all(link , value):
    ...
    """
    "*** YOUR CODE HERE ***"
    #becaues it seems hard to modify the .first, so let's always try on the .rest
...
def deep_map(f, link):
    ...
    "*** YOUR CODE HERE ***"
    # import copy
    # # if not isinstance(link_to_go,Link):
    # link_to_go = copy.deepcopy(link)
    # if not isinstance(link.first, Link):
    #     link_to_go.first = f(link.first)
    #     print(link.first)
    #     if link.rest:
    ...
    # learned from others, because it is recursion, so the deepcopy way above won't work
    # because it will be deep copied every recursion.
    # so the though to evaluate a new instance in the return statement is really awsome!!
    # and maybe it is the only way.
