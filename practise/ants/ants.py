...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        # while not place_to_check.bees:
        #     if place_to_check.entrance.bees is beehive.bees:
        #         # return None
        #         break
        #     else:
        ...
        place_to_check = self.place
        distance_counter = 0
        while not place_to_check.bees or distance_counter < self.min_range:
            if place_to_check.entrance.bees is beehive.bees:
                # return None
                break
            elif distance_counter+1 > self.max_range:
                break
            else:
                place_to_check = place_to_check.entrance
                distance_counter += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor > amount:
            ...
            added_damage = self.damage
        for bee in list(self.place.bees):
            bee.reduce_armor(amount + added_damage)
...
# The WallAnt class
class WallAnt(Ant):
    '''Wallant do nothing but stand like a wall.'''
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        self.armor = armor
        self.queen_number = QueenAnt.queen_count
        ...
        if self.queen_number > 0:
            ...
            while place_to_check:
                if place_to_check.ant:
                    if place_to_check.ant.is_container and \
                        place_to_check.ant.contained_ant and \
                         place_to_check.ant.contained_ant not in self.buffed_ants:
                        self.buffed_ants.append(place_to_check.ant.contained_ant)
                        place_to_check.ant.contained_ant.damage *=2
                    elif place_to_check.ant not in self.buffed_ants:
                        self.buffed_ants.append(place_to_check.ant)
                        place_to_check.ant.damage *= 2
    ...
    def reduce_armor(self, amount):
        ...
        if self.queen_number == 0:
            self.armor -= amount
            if self.armor <= 0 and self.queen_number == 0:
                bees_win()
        else:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        discance = 0
        while place_to_check:
            if place_to_check.bees:
                for bee in place_to_check.bees:
                    insects_dic[bee] = discance
            elif place_to_check.ant:
                insects_dic[place_to_check.ant] = discance
                if place_to_check.ant.is_container \
                    and place_to_check.ant.contained_ant:
                    insects_dic[place_to_check.ant.contained_ant] = discance
            discance, place_to_check = discance + 1, place_to_check.entrance
        return insects_dic
        # END Problem OPTIONAL

    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        damage_result = self.damage - distance*0.2
