...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(my_operator)
        my_operands = rest.map(lambda x : scheme_eval(x, env)) # it is not a 'real' mapping in traditional way.
...
def eval_all(expressions, env):
    ...
    # BEGIN PROBLEM 7
    # eval_ed = expressions.map(lambda x : scheme_eval(x.first, env))
    res = None
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        fn = LambdaProcedure(target.rest, expressions.rest, env)
        env.define(target.first, fn)
        return target.first
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    return do_quasiquote_form(expressions, env)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    my_formals, my_vals = nil, nil
    while bindings is not nil:
        check_form(bindings.first, 2, 2)
        my_formals = Pair(bindings.first.first, my_formals)
        my_vals = Pair(scheme_eval(bindings.first.rest.first, env), my_vals)
        bindings = bindings.rest
        check_formals(my_formals)
    return env.make_child_frame(my_formals, my_vals)
    # END PROBLEM 14
